...
function renderTextWithLinks(text: string) {
  ...
  while ((match = regex.exec(text)) !== null) {
    ...
    if (linkUrl.startsWith('/')) {
      parts.push(
        <Link key={match.index} href={linkUrl} className="text-accent-orange font-bold hover:underline">
          {linkText}
        </Link>
      );
    } else {
      parts.push(
        <a key={match.index} href={linkUrl} target="_blank" rel="noopener noreferrer" className="text-accent-orange font-bold hover:underline">
          {linkText}
        </a>
      );
    }
...
export async function generateMetadata({ params }: PageProps) {
  ...
  if (!post) {
    return {
      title: "Article Not Found | Optima Safety Solutions",
    };
  }

  return {
    title: `${post.title} | Optima Blog`,
    ...
    openGraph: {
      title: post.title,
      description: post.summary,
      url: `https://www.optimasafetysolutions.com/blog/${resolvedParams.slug}`,
      images: [
        {
          url: post.image,
          width: 1200,
...
export async function generateStaticParams() {
  ...
  return posts.map((post: BlogArticle) => ({
    slug: post.slug,
  }));
}
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-xs flex flex-col gap-8">
              ...
              <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200 flex flex-col gap-3">
                ...
                <ul className="space-y-2 text-xs sm:text-sm text-primary-700 font-bold">
                  {post.contentBlocks.map((block: { heading: string; text: string }, i: number) => {
              ...
              <div className="flex flex-col gap-8">
                {post.contentBlocks.map((block: { heading: string; text: string }, idx: number) => {
