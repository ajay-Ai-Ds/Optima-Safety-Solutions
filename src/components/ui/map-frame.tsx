...
export default function MapFrame({ areaName = "Chennai", className = "" }: MapFrameProps) {
  ...
  const queryLocation = areaName.toLowerCase().includes("chennai") ? areaName : `${areaName}, Chennai, Tamil Nadu, India`;
  const mapSearchUrl = `https://maps.google.com/maps?q=${encodeURIComponent(queryLocation)}&t=&z=13&ie=UTF8&iwloc=&output=embed`;

  return (
    <div className={`relative w-full h-[350px] rounded-3xl overflow-hidden shadow-xs border border-slate-200 ${className}`}>
      <iframe
        title={`Optima Safety Solutions Service Coverage Map - ${areaName}`}
        src={mapSearchUrl}
        width="100%"
        height="100%"
        style={{ border: 0 }}
